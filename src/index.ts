...
async function main(): Promise<void> {
  ...
  if (DISCORD_BOTS.length > 0) {
    // Multi-bot mode: each bot uses dc:BOTNAME:CHANNEL_ID JID format
    for (const bot of DISCORD_BOTS) {
      const discord = new DiscordChannel(bot.token, channelOpts, bot.name);
      channels.push(discord);
      await discord.connect();
    }
  } else if (DISCORD_BOT_TOKEN) {
    // Legacy single-bot mode: dc:CHANNEL_ID JID format
  ...
  startIpcWatcher({
    ...
    reactToMessage: async (jid, messageId, emoji) => {
      const channel = findChannel(channels, jid);
      if (!channel) throw new Error(`No channel for JID: ${jid}`);
      if (!channel.reactToMessage) throw new Error(`Channel ${channel.name} does not support reactToMessage`);
